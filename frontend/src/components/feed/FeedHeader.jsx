...
export function FeedHeader({ companies, itemCount, onToggleSidebar, onOpenSettings }) {
  ...
  return (
    <>
      {/* Desktop header */}
      <div className="hidden md:flex relative flex-row items-center gap-2 px-3 h-11 border-b border-white/10 bg-background flex-shrink-0">
        {/* Left */}
        ...
        <div
          className={cn(
            detailActive
              ? "flex-1 min-w-0 mx-2"
              : "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[30vw] lg:w-[30vw]"
          )}
        >
          <SearchBar
            value={search}
      ...
      <div className="md:hidden flex flex-col bg-background border-b border-white/10 flex-shrink-0">
        {/* Row 1: Hamburger + Companies + Settings */}
        <div className="flex items-center justify-between px-3 h-11 border-b border-white/10">
          <button
            onClick={onToggleSidebar}
            className="p-2 -ml-2 rounded-md transition-colors hover:bg-black/[0.05] dark:hover:bg-white/[0.08] flex-shrink-0"
            aria-label="Toggle sidebar"
          >
          ...
          <button
            ...
            className="p-2 -mr-2 rounded-md transition-colors hover:bg-black/[0.05] dark:hover:bg-white/[0.08] flex-shrink-0"
            aria-label="Settings"
          >
            <Settings size={18} />
          </button>
        ...
        {/* Row 2: Date toggle + Tag filter */}
        <div className="flex items-center justify-between px-3 h-10 border-b border-white/10 gap-2">
          <ScopeToggle
            value={settings.scope}
            onChange={(scope) => updateSettings({ scope })}
          />
